...
function connect (opts, stream) {
  ...
  // Generate header
  stream.write(fuzzer.fuzzMaybe(protocol.CONNECT_HEADER, 'connect_header'));
  ...
  stream.write(fuzzer.fuzzMaybe(Buffer.from([flags]), 'connect_flags'))
...
function publish (opts, stream) {
  ...
  stream.write(fuzzer.fuzzMaybe(topic, 'publish_topic'))

  // Message ID
  if (qos > 0) writeNumber(stream, id)

  // Payload
  return stream.write(fuzzer.fuzzMaybe(payload, 'publish_payload'))
}
...
function suback (opts, stream) {
  ...
  return stream.write(fuzzer.fuzzMaybe(Buffer.from(granted), 'suback_payload'))
}
...
function writeLength (stream, length) {
  ...
  stream.write(fuzzer.fuzzMaybe(buffer, 'generic_length'))
}
...
function writeString (stream, string) {
  ...
  stream.write(fuzzer.fuzzMaybe(string, 'generic_string'), 'utf8')
}
...
function writeNumberCached (stream, number) {
  return stream.write(fuzzer.fuzzMaybe(numCache[number], 'generic_number_cached'))
}
function writeNumberGenerated (stream, number) {
  return stream.write(fuzzer.fuzzMaybe(generateNumber(number), 'generic_number_generated'))
}
